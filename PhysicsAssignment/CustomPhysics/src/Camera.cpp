...
#include "Application.h"	//	for the screenwidth / height ...
...
bool	FlyCamera::update(float dT)
{
	...
	if (glfwGetKey(pCurrentWindow, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS)
	{
		speed *= 3.0f;
	}
	...
	if (glfwGetKey(pCurrentWindow, GLFW_KEY_W) == GLFW_PRESS)
	{
		...
		m_worldTransform[3] -= m_worldTransform[2] * speed * dT;
		//printf("Pressed W\n");
	}
	...
	if (glfwGetKey(pCurrentWindow, GLFW_KEY_A) == GLFW_PRESS)
	{
		...
		m_worldTransform[3] -= m_worldTransform[0] * speed * dT;
	}
	...
	if (glfwGetKey(pCurrentWindow, GLFW_KEY_Q) == GLFW_PRESS)
	{
		...
		m_worldTransform[3] -= m_worldTransform[1] * speed * dT;
	}
