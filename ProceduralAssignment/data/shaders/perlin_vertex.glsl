...
void main()
{
	...
	float fDeltaX = 0.005f;
	float fDeltaZ = 0.005f;
	vec2 newTexCoord;
	newTexCoord = texcoord;
	newTexCoord.x += fDeltaX;
	vec3 vTangentX;
	vTangentX.x = pos.x + fDeltaX;
	...
	vTangentZ.z = pos.z + fDeltaZ;
	frag_bitangent = vTangentZ - pos.xyz;

	vec3 tangentCross = cross(frag_tangent, frag_bitangent);
