...
bool	PhysicsApp::startup()
{
	...
	m_FlyCamera = FlyCamera(vec3(0, 10, 25), vec3(0, 10, 0), glm::radians(50.0f), 1280.0f / 720.0f, 0.1f, 1000.0f);
	...
	//	ok, so let's start initialising the physics scene!
	for (int i = 0; i < 10; ++i)
	{
		BBPhysicsObject*	pPhysObj = new BBSphere(vec2(i - 5, i + 5), vec2(0.1f * (5 - i), 0.20f), i + 5, 0.5f, vec4(0.0f, 0.05f * i, 0.1f * i, 1.0f));
		m_oBBPhysics.TheScene.AddActor(pPhysObj);
	}
...
bool	PhysicsApp::update()
{
	...
	m_oBBPhysics.TheScene.update(dT);
...
void	PhysicsApp::draw()
{
	...
	m_oBBPhysics.TheScene.AddGizmos();
